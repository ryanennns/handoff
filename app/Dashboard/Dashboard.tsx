...
import { FaSpotify, FaYoutube, FaArrowRight, FaCheck, FaClock, FaExclamationTriangle } from "react-icons/fa";
...
const StatusBadge: React.FC<{ status: TransferJob["status"] }> = ({ status }) => {
  const statusConfig = {
    pending: { icon: <FaClock />, text: "Pending", color: "bg-yellow-500" },
    "in-progress": { icon: <FaClock />, text: "In Progress", color: "bg-blue-500" },
    completed: { icon: <FaCheck />, text: "Completed", color: "bg-green-500" },
    failed: { icon: <FaExclamationTriangle />, text: "Failed", color: "bg-red-500" },
  };
...
export const Dashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "new-transfer">("overview");
  ...
  const services: ServiceOption[] = [
    { id: "spotify", name: "Spotify", icon: <FaSpotify />, color: "text-green-400" },
    { id: "youtube", name: "YouTube Music", icon: <FaYoutube />, color: "text-red-500" },
    { id: "apple", name: "Apple Music", icon: <SiApplemusic />, color: "text-red-400" },
    { id: "tidal", name: "Tidal", icon: <SiTidal />, color: "text-white" },
    { id: "amazon", name: "Amazon Music", icon: <SiAmazonmusic />, color: "text-cyan-400" },
  ];
  ...
  const handleStartTransfer = () => {
    ...
    console.log("Starting transfer:", { fromService, toService, selectedPlaylist });
    // Reset form
    setFromService("");
    setToService("");
    setSelectedPlaylist("");
            ...
            {activeTab === "overview" ? (
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    {[
                      { label: "Total Transfers", value: "847", color: "from-green-400 to-cyan-400" },
                      { label: "Songs Transferred", value: "23,451", color: "from-pink-400 to-purple-400" },
                      { label: "Active Jobs", value: "3", color: "from-yellow-400 to-orange-400" },
                      { label: "Success Rate", value: "98.7%", color: "from-blue-400 to-indigo-400" },
                    ].map((stat) => (
                        <div
                            key={stat.label}
                            ...
                            >
                              <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-4">
                                  ...
                                  <div>
                                    <h3 className="text-white font-semibold text-lg">{job.playlistName}</h3>
                                  ...
                                  {job.status === "in-progress" && (
                                      <div className="w-32 bg-white/20 rounded-full h-2">
                                        <div
                                            ...
                                            style={{ width: `${(job.songsTransferred / job.songsTotal) * 100}%` }}
                                        />
                                      </div>
                                  )}
                ...
                <div className="max-w-4xl mx-auto">
                  <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-8 border border-white/20">
                    <h2 className="text-3xl font-bold text-white mb-8 text-center">Start New Transfer</h2>
                    ...
                    <div className="space-y-8">
                      ...
                      {fromService && toService && fromService !== toService && (
                          <div className="space-y-3">
                            <label className="text-white font-medium text-lg">Select Playlist</label>
                            <select
                      ...
                      <div className="flex justify-center pt-6">
                        <button
                            ...
                            disabled={!fromService || !toService || !selectedPlaylist || fromService === toService}
