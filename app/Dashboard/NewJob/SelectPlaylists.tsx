...
export const SelectPlaylists = ({ service, setter }: Props) => {
  ...
  useEffect(() => {
    setter(selectedPlaylists);
  }, [selectedPlaylists]);

  const isSelected = (name: string) =>
    selectedPlaylists.some((p) => p.name === name);

  const togglePlaylist = (playlist: Playlist) => {
    setSelectedPlaylists((prev) =>
      isSelected(playlist.name)
        ? prev.filter((p) => p.name !== playlist.name)
        : [...prev, playlist],
    );
  };
  ...
  const selectAll = () =>
    setSelectedPlaylists(
      selectedPlaylists.length === playlists.length ? [] : [...playlists],
    );
    ...
    return (
      <>
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-white">2. Select Playlists</h2>
        </div>
  ...
  if (!playlists || playlists.length === 0 || !service) {
    return (
      <>
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-white">2. Select Playlists</h2>
      ...
      <div className="flex flex-row justify-between items-center mb-6 gap-4">
        ...
        <div className="flex-1 p-4 bg-white/5 rounded-2xl border border-white/10">
          <input
            ...
            className="w-full h-full bg-transparent text-white placeholder-white/40 focus:outline-none"
            onChange={(e) => setSearchTerm(e.target.value)}
          />
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 max-h-96 overflow-y-auto">
        {playlists
          .filter((p) =>
